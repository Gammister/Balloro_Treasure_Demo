(function initPuckLuckMath(root, factory) {
  ...
  const EMPTY_BONUS_PRESENTATION_MULTIPLIER = 2;
  ...
  const SHAPES = {
    low: { outer: 1, middle: 1.35, center: 1.8 },
    normal: { outer: 1, middle: 1.8, center: 3.5 },
    high: { outer: 0.05, middle: 1, center: 10 }
  };
  const PROFILE = {
    low: { hit: 0.64, shares: { outer: 0.55, middle: 0.35, center: 0.1 }, bonusShare: 0.05 },
    normal: { hit: 0.42, shares: { outer: 0.48, middle: 0.37, center: 0.15 }, bonusShare: 0.08 },
    high: { hit: 0.2, shares: { outer: 0.5, middle: 0.32, center: 0.18 }, bonusShare: 0.14 }
  };
  ...
  function buildSectorDefinitions(lines) {
    ...
    for (let row = 0; row < lines; row += 1) {
      for (let col = 0; col < lines; col += 1) {
        ...
        const distance = Math.max(Math.abs(col - center), Math.abs(row - center));
        const closeness = 1 - distance / maxDistance;
        if (closeness >= 0.82) sectors.center.push(sector);
        else if (closeness >= 0.42) sectors.middle.push(sector);
        else sectors.outer.push(sector);
      }
    }
  ...
  function solveConfiguration(risk, lines, pucks) {
    ...
    const bonusProbability = desiredBonusShare / (9 * (1 - desiredBonusShare));
    const baseTarget = TARGET_RTP / (1 + 9 * bonusProbability);
    const shape = SHAPES[risk];
    const weightedShape = profile.shares.outer * shape.outer
      + profile.shares.middle * shape.middle
      + profile.shares.center * shape.center;
    const scale = baseTarget / (profile.hit * weightedShape);
    const multipliers = {
      empty: 0,
      outer: round2(shape.outer * scale),
      middle: round2(shape.middle * scale),
      center: round2(shape.center * scale)
    };
    ...
    return {
      ...
      desired_bonus_share: desiredBonusShare,
      solver: { shape, scale, calibrated_hit_probability: calibratedHit }
    };
  }
  ...
  function createRound({ risk, lines, pucks, betPerPuck = 1, seed = 1 }) {
    ...
    const baselineCollectedCount = paidBonusTriggered ? pucks : rng.int(pucks);
    const puckResults = [];
    ...
    const baseEmptyBonusChance = Math.max(0,
      (BONUS_PRESENTATION_TARGET_RATE - config.bonus_probability)
  ...
  function simulateConfiguration(config, totalRuns = 1000000, seed = hashString(config.id)) {
